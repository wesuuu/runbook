<script lang="ts">
    ...
    onMount(async () => {
        try {
            ...
            loadError = err instanceof Error ? err.message : 'Failed to load sites';
        } finally {
            loading = false;
        }
    });
    ...
    async function setAsDefault(siteId: string): Promise<void> {
        try {
            const updated = await api.post<Site>(
                `/sites/${siteId}/set-default`,
                {},
                { schema: SiteSchema },
            );
            sites = sites.map((s) => {
                if (s.id === updated.id) return updated;
                if (s.is_default) return { ...s, is_default: false };
                return s;
            });
            toast.success(`${updated.name} is now the default site`);
        } catch (err) {
            const msg = err instanceof Error ? err.message : 'Failed to set default';
            toast.error('Set default failed', msg);
        }
    }
...
{#if loading}
    <div class="p-6 text-sm text-muted-foreground">Loading sites…</div>
{:else if loadError}
    <div class="p-6 text-sm text-destructive">{loadError}</div>
{:else}
<div class="grid grid-cols-12 min-h-[70vh] border border-border rounded-md overflow-hidden bg-card" in:fade={{ duration: 120 }}>
    <SiteList
        ...
        {:else}
            <header class="px-6 py-4 flex items-center justify-between border-b border-border">
                ...
                <div class="flex items-center gap-2">
                    {#if adminFlag && !activeSiteIsDefault}
                        <Button
                            variant="outline"
                            size="sm"
                            onclick={() => setAsDefault(activeSite!.id)}
                        >
                            Set as default
                        </Button>
                    {/if}
