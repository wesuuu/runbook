...
{#if panelState === "open"}
    ...
    >
        <!-- Header -->
        <div
            class="flex items-center justify-between px-4 py-3 border-b border-border/40 bg-card/50 flex-shrink-0"
        >
        ...
        <div class="flex-1 overflow-y-auto px-4 py-3 space-y-3">
            {#if !activeSession || activeSession.messages.length === 0}
                <div class="h-full flex items-center justify-center">
                    <div class="text-center px-4">
                        <div
                            class="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center mx-auto mb-3"
                        >
                            <svg
            ...
            {:else}
                {#each activeSession.messages as msg (msg.id)}
                    {#if msg.role === "assistant" && pendingApproval && msg.id === pendingApproval.assistant_message_id}
                        ...
                        >
                            {#if msg.role === "assistant"}
                                {#if Array.isArray(msg.metadata_?.tool_calls)}
                                    <div
                                        class="flex flex-col gap-1 mb-2 pb-2 border-b border-border/30"
                                    >
                                        {#each msg.metadata_.tool_calls as tc_raw}
                                            {@const tc = tc_raw as Record<
                                                string,
                                                unknown
                                            >}
                                            <span
                                                class="text-[11px] text-muted-foreground/70 flex items-center gap-1.5 italic"
                                            >
                                                <svg
                                                ...
                                                {#if tc.tool === "search_documents"}
                                                    Searched documents for "{tc.query}"
                                                    ({tc.results} result{tc.results !==
                                                    1
                                                        ? "s"
                                                        : ""})
                                                ...
                                                {:else if tc.tool === "list_unit_ops"}
                                                    Listed unit operations ({tc.results}
                                                    available)
                                ...
                                {/if}
                                <div
                                    class="prose prose-sm max-w-none dark:prose-invert chat-prose"
                                >
        ...
        <div class="border-t border-border/40 p-3 bg-card/30 flex-shrink-0">
            ...
            <p class="text-[10px] text-muted-foreground text-center mt-1.5">
                <kbd class="px-1 py-0.5 rounded bg-muted text-[9px] font-mono"
                    >{shortcutLabel}+J</kbd
                > to toggle
            </p>
        </div>
...
<style>
    ...
    .chat-panel {
        animation: chat-panel-in 0.25s ease-out both;
    }
