...
def build() -> Document:
    ...
    _apply_font(title_p.add_run("{{ protocol_name }}"),
                size=18, bold=True, color=TEXT_BLACK)

    sub_p = doc.add_paragraph()
    sub_p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    sub_p.paragraph_format.space_after = Pt(2)
    _apply_font(sub_p.add_run("Batch Manufacturing Record"),
                size=11, italic=True, color=TEXT_MUTED)

    org_p = doc.add_paragraph()
    org_p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    org_p.paragraph_format.space_after = Pt(8)
    _apply_font(org_p.add_run("{{ organization_name }}"),
    ...
    rows_data = [
        ...
        ("Target Yield / Scale", "{{ target_yield }}"),
    ...
    _add_para(
        ...
        size=9, color=TEXT_MUTED, italic=True, space_after=4,
    )
    ...
    for ci, h in enumerate(bom_headers):
        _style_cell(t2.rows[0].cells[ci], h, bold=True, size=10,
                    color=TEXT_BLACK, fill=HEADER_FILL)
    _style_cell(t2.rows[1].cells[0], "{%tr for material in materials %}",
    ...
    for ci, h in enumerate(eq_headers):
        _style_cell(t3.rows[0].cells[ci], h, bold=True, size=10,
                    color=TEXT_BLACK, fill=HEADER_FILL)
    _style_cell(t3.rows[1].cells[0], "{%tr for eq in equipment %}",
                size=8, color=TEXT_MUTED)
    ...
    def _emit_step_table(loop_expr: str):
        ...
        for ci, h in enumerate(step_headers):
            _style_cell(t.rows[0].cells[ci], h, bold=True, size=10,
                        color=TEXT_BLACK, fill=HEADER_FILL)
        _style_cell(t.rows[1].cells[0], loop_expr, size=8, color=TEXT_MUTED)
        _style_cell(t.rows[2].cells[0], "{{ loop.index }}", size=10,
        ...
        cell_val.text = ""
        run_val = cell_val.paragraphs[0].add_run("{{r step.value_display }}")
        _apply_font(run_val, size=10)
        _set_cell_border(cell_val)
        cell_val.vertical_alignment = WD_ALIGN_VERTICAL.CENTER
        _style_cell(t.rows[2].cells[4], "{{ step.initials }}", size=10,
                    align=WD_ALIGN_PARAGRAPH.CENTER)
        _style_cell(t.rows[2].cells[5], "", size=10)
        _style_cell(t.rows[3].cells[0], "{%tr endfor %}", size=8,
                    color=TEXT_MUTED)
    ...
    _apply_font(
        ...
        size=11, bold=True, color=TEXT_BLACK,
    )
    ...
    _add_para(
        ...
        size=9, color=TEXT_MUTED, italic=True, space_after=6,
    )

    # System-captured approval block
    ...
    _add_para(
        ...
        size=9, color=TEXT_BLACK, italic=True,
    )
    _add_jinja_para(doc, "{%p endif %}")
    _add_jinja_para(doc, "{%p if approval.signature_image %}")
    sig_p = doc.add_paragraph()
    _apply_font(
        sig_p.add_run("Signature: {{ approval.signature_image }}"),
        size=10,
    )
    _add_jinja_para(doc, "{%p endif %}")
    _add_jinja_para(doc, "{%p endif %}")

    # Approval history audit table
    ...
    _set_table_layout(t6, [90, 130, 200, 80])
    sig_headers = ["Action", "Name", "Signature Statement", "Date"]
    for ci, h in enumerate(sig_headers):
        _style_cell(t6.rows[0].cells[ci], h, bold=True, size=10,
                    color=TEXT_BLACK, fill=HEADER_FILL)
    _style_cell(t6.rows[1].cells[0], "{%tr for ev in approval_history %}",
                size=8, color=TEXT_MUTED)
    ...
    for ci, h in enumerate(att_headers):
        _style_cell(t8.rows[0].cells[ci], h, bold=True, size=10,
                    color=TEXT_BLACK, fill=HEADER_FILL)
    _style_cell(t8.rows[1].cells[0],
                "{%tr for att in non_image_attachments %}", size=8,
                color=TEXT_MUTED)
