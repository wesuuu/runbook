...
async def test_set_default_swaps_default_atomically(
    ...
    res = await authed_admin_client.post(
        f"/sites/{unmanaged_site.id}/set-default"
    )
...
@pytest.mark.asyncio
async def test_set_default_site_manager_403(
    authed_site_manager_client, managed_site
):
    """SITE_MANAGER role + grant is NOT enough — only ADMIN can promote."""
    res = await authed_site_manager_client.post(
        f"/sites/{managed_site.id}/set-default"
    )
    assert res.status_code == 403
