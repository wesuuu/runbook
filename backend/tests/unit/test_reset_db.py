...
def test_assert_local_dev_db_accepts_127_0_0_1():
    assert_local_dev_db(
        "postgresql+asyncpg://postgres:postgres@127.0.0.1:5432/batchrite"
    )
...
def test_confirm_reset_accepts_lowercase_y():
    with patch("sys.stdin") as fake_stdin, patch("builtins.input", return_value="y"):
...
def test_confirm_reset_rejects_non_y_answers(answer):
    with patch("sys.stdin") as fake_stdin, patch("builtins.input", return_value=answer):
        fake_stdin.isatty.return_value = True
