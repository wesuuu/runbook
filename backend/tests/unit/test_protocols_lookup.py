...
@pytest.mark.asyncio
async def test_list_protocols_includes_org_admin_visibility(
    db_session: AsyncSession, test_org: Organization, test_user: User
):
    """Org admins see protocols in projects with NO explicit ObjectPermission."""
    # test_user is an ADMIN of test_org (per conftest fixture).
    # Create a project with no ObjectPermission rows whatsoever.
    proj = Project(
    ...
    items = await list_protocols(db_session, user_id=test_user.id, org_id=test_org.id)
    assert any(
        it.name == "AdminVisible" for it in items
    ), "Org admin should see protocols even without explicit ObjectPermission"


@pytest.mark.asyncio
async def test_list_protocols_includes_team_permission_visibility(
    db_session: AsyncSession, test_org: Organization
):
    """Non-admin users see protocols via TEAM-principal permissions."""
    # Create a non-admin member in test_org.
    member = User(
    ...
    # Team they belong to.
    team = Team(name="T1", organization_id=test_org.id)
    db_session.add(team)
    await db_session.flush()
    db_session.add(TeamMember(team_id=team.id, user_id=member.id))
    # Project with permissions_enabled=true (so it's NOT open to all members)
    # and a TEAM-principal permission grant.
    proj = Project(
        name="team-proj",
        organization_id=test_org.id,
        owner_id=uuid.uuid4(),
    ...
    items = await list_protocols(db_session, user_id=member.id, org_id=test_org.id)
    assert any(
        it.name == "TeamVisible" for it in items
    ), "Member should see protocols visible via TEAM permission grant"
